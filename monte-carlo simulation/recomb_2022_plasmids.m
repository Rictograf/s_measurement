...
%% Recombination of randomly chosen pairs with one-parent replacement
    ...
    %% Horizontal transfer H - probability to give/take gene fragment to/from shared genetic pool. B - amount of gene blocks per genome
thoseWhoGive = rand(N,1) < H;

thoseWhoTake = rand(N,1) < H;
% Gene pool template construction
GenePool = cell(B,1);
for i = 1:B
    GenePool{i} = [];
end
...
for i = 1:N
    if thoseWhoGive(i)
        currentGenome = K(i,:);
        targetedBlock = randi(B);
        NewGene = currentGenome(1+blockSize*(targetedBlock-1):blockSize*targetedBlock);
        GenePool{targetedBlock} = [GenePool{targetedBlock}; NewGene];
    end
end
...
for i = 1:N
    if thoseWhoTake(i)
       targetedBlock = randi(B);
       % Randomly choice one chunk and change if it is better than current
       chunkmaxsize = size(GenePool{targetedBlock});
       if chunkmaxsize>0
       chunk = randi(chunkmaxsize(1));
       %chunk = randi(length(GenePool{targetedBlock}));
       if sum((K(i,1+blockSize*(targetedBlock-1):blockSize*targetedBlock)).*s(1+blockSize*(targetedBlock-1):blockSize*targetedBlock))<sum((GenePool{targetedBlock}(chunk,:)).*s(1+blockSize*(targetedBlock-1):blockSize*targetedBlock))
           %{
           disp(chunk)
           disp("This one is better")
           % check whether it is true
           disp("Is it though?")
           disp("K is")
           disp(K(i,1+blockSize*(targetedBlock-1):blockSize*targetedBlock))
           disp("Chunk is")
           disp(GenePool{targetedBlock}(chunk,:))
           disp("s is")
           disp(s(1+blockSize*(targetedBlock-1):blockSize*targetedBlock))
           %}
           K(i,1+blockSize*(targetedBlock-1):blockSize*targetedBlock) = GenePool{targetedBlock}(chunk,:);
       end
       end
       % Next code is legacy, left here just in case
       %{
       % Think about behavior. Right now - tries to find the best one, if
       % can't - stops
       % Change simple sum to multiplication by s and summ
       chunk = 1;
       disp(length(GenePool{targetedBlock}))
       while chunk<=length(GenePool{targetedBlock})
           if sum(K(i,1+blockSize*(targetedBlock-1):blockSize*targetedBlock))<sum(GenePool{targetedBlock}(chunk,:))
                disp("found a better one")
                K(i,1+blockSize*(targetedBlock-1):blockSize*targetedBlock) = GenePool{targetedBlock}(chunk,:);
                break
           end
           chunk = chunk +1;
